...
class NtripClient:
    ...
    def read_gnss_log(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if message:
                    logging.info(f"Received GNSS message: {message.decode()}")
                else:
    ...
    def send_gnss_cmd(self, command):
        """Send a command to the GNSS receiver."""
        if self.gnss_socket:
            try:
                self.gnss_socket.send(command.encode())
                logging.info(f"Sent command to GNSS receiver: {command}")
            except Exception as e:
    ...
    def run(self):
        ...
        if self.connect_ntrip_server():
            ...
            try:
                ...
                self.gnss_log_thread = threading.Thread(target=self.read_gnss_log, daemon=True)
